...
#def make_lmdb(dataset, lmdb_file, num_workers=0):   
def make_lmdb(dataset_s1, dataset_s2, lmdb_file, num_workers=0):     
    assert len(dataset_s1) == len(dataset_s2), "S1 and S2 datasets must have the same length"
    loader_s1 = InfiniteDataLoader(dataset_s1, num_workers=num_workers, collate_fn=lambda x: x[0])
    loader_s2 = InfiniteDataLoader(dataset_s2, num_workers=num_workers, collate_fn=lambda x: x[0])
    ...
    env = lmdb.open(lmdb_file, map_size=16492674416*3)
    
    
    txn = env.begin(write=True)
    for index, ((sample_s1, target_s1), (sample_s2, target_s2)) in tqdm(enumerate(zip(loader_s1, loader_s2)), total=len(dataset_s1), desc='Creating LMDB'):
        assert target_s1.tobytes() == target_s2.tobytes(), "Targets for S1 and S2 must be the same"
        
        sample_s1 = np.array(sample_s1)
        sample_s2 = np.array(sample_s2)
        obj = (sample_s1.tobytes(), sample_s1.shape, sample_s2.tobytes(), sample_s2.shape, target_s1.tobytes())
...
class LMDBDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #这样尝试的原因是float大小不匹配，所以尝试uint8
        #print(f"sample_s1_shape: {sample_s1_shape}")  # 打印 sample_s1_shape
        sample_s1 = np.frombuffer(sample_s1_bytes, dtype=np.uint8).reshape(sample_s1_shape)
        #为了能输出让s1=s2
        sample_s2 = sample_s1
