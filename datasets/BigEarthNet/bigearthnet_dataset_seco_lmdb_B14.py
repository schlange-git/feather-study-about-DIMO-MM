...
class LMDBDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            ...
            '''
            print("Number of elements:", len(loaded_data))
            print("Types of elements:", [type(x) for x in loaded_data])

            # 打印每个元素的部分信息
            for index, item in enumerate(loaded_data):
                if isinstance(item, bytes):
                    print(f"Element {index}: bytes, length={len(item)}")
                elif isinstance(item, tuple):
                    print(f"Element {index}: tuple, content={item}")
                else:
                    print(f"Element {index}: {type(item)}")
            '''
